...
class DocumentosController extends Controller
{
    ...
    public function create()
    {
		...
		if (Auth::check()) {
			...
			$establecimientos = Establecimiento::where('active',1)->get();
			return view('factura.oficinaPartes.create',compact('tipos','establecimientos')); 
		}
		else {
    ...
    public function store(Request $request)
    {
		if (Auth::check()) {
			...
			$validator = validator::make($request->all(), [
				...
				'establecimiento' => 'required', // para identificar el destinatario del documento *
			...
			if ($validator->fails()) {
				...
				$documento->establecimiento_id = $estab_user; // para identificar el destinatario de documento *
				...
				$documento->destino_id = $request->establecimiento; // destino de los documentos
	...
	public function adjuntarValidadores2(Request $request)
	{
		if (Auth::check()) {
			...
			$documentos =  DB::table('documentos')
							...
							->whereIn('movimientos.estado',['DE','RC','RT','VB','RE'])		
							->orderBy('documentos.id','desc')->paginate(10)
							->appends('searchRut',$request->get('searchRut'))
							->appends('searchTipo',$request->get('searchTipo'))
							->appends('searchNdoc',$request->get('searchNdoc'));
	 ...
	 * @return \Illuminate\Http\Response
	 */	 
	public function formularioAdjuntar(Request $request, $id, $estado) 
	{	
		//dd($estado);
		//$todos = $request->all();
		//$test = $request->only('json', 'requestUri');
		//dd($test);
		$estado = $estado;
		...
		if (Auth::check()) {
			...
			if ( $documento->establecimiento_id == $estab_user ) {	
				...
				$movimientos = DB::table('movimientos')
							   ->join('documentos', 'documentos.id','=','movimientos.documento_id')
							   ->where(
							   		[
										['movimientos.active','=',1],
										['movimientos.documento_id','=',$id]
									]
								)
							   ->select(
							   		'movimientos.documento_id as documento_id',
							   		'movimientos.estado as estado',
							   		'movimientos.observacion as observacion',
							   		'documentos.nDoc as nDoc'
							   	)
							   ->orderBy('movimientos.documento_id')->first();
				//dd($movimientos->estado);
				
				return view('factura.oficinaConvenios.formularioAdjuntar',compact('documento','proveedor','tipo','validadores','validadors','movimientos'));
			}
	...
	public function reporte(Request $request) 
	{
		if (Auth::check()) {
			...
			return view('factura.reporte',compact('tipos','referentes','establecimientos'));	
		}
		else {
			return view('auth/login');
		}
		...
		if (Auth::check()) {
			...
			$estab      = $user->establecimientos()->first()->id;
						...
						$documentos =  DB::table('documentos')
							...
							->join('tipo_docs','documentos.tipoDoc_id','=','tipo_docs.id')
							//->join('establecimientos','documentos.establecimiento_id','=','establecimientos.id')
							->join('establecimientos as e1','documentos.establecimiento_id','=','e1.id') 
							->join('establecimientos as e2','documentos.destino_id','=','e2.id')
							->leftjoin('tipo_pagos','documentos.pago_tipo_id','=','tipo_pagos.id')
							->leftjoin('convenios','documentos.convenio_id','=','convenios.id')
							->leftjoin('referentes','convenios.referente_id','=','referentes.id')
							->orderBy('documentos.id','desc')
							->select('proveedors.rut as rut',
									 ...
									 'documentos.pago_operacion as pago_operacion',
									 'documentos.pago_sigfe as pago_sigfe',
									 //'documentos.destino_id as destino_id',
									 //'establecimientos.name as establecimiento',
									 'e1.name as establecimiento',
									 'e2.name as destino',										 
									 'convenios.identificador as convenio',
									 'referentes.name as referente',
									 'tipo_pagos.name as tipoPago',
			...
			if($request->get('establecimiento') != null){
				$documentos = $documentos->where('documentos.destino_id',$request->get('establecimiento'));
			}


			//,['documentos.destino_id',$request->get('establecimiento')]
	...
	public function excel(Request $request) 
	{
		if (Auth::check()) {
			...
			//crea array con información de consulta
			$documentosArray[] = ['Proveedor','Tipo de Documento','Nro de Documento','Nomina','Monto','Fecha de Emisión','Fecha de Recepción','Referente Técnico','Convenio','Establecimiento','Estado','Destino'];
